 ...
 * Renders a visual waveform using Canvas for performance.
 * Simulates deterministic waveform data based on clip ID to avoid expensive decoding in prototype.
 */
const ClipWaveform: React.FC<{ clip: Clip; width: number; height: number; color: string }> = ({ clip, width, height, color }) => {
  ...
  useEffect(() => {
    ...
    // Generate deterministic "random" peaks based on ID
    const peaks = Math.ceil(width / 2); // One bar every 2px
    ...
    for (let i = 0; i < peaks; i++) {
       ...
       // Amplitude modulation (mock envelope)
       const amplitude = noise * (height * 0.8); 
       
       // Draw mirrored bar
       const barHeight = Math.max(2, amplitude); // Min height 2px
 ...
 * Visual representation of the volume level overlay
 */
const VolumeLine: React.FC<{ volume: number }> = ({ volume }) => {
    // Volume 0.0 -> Bottom (100%), Volume 1.0 -> Top (20% margin)
                ...
                return (
                  <div key={track.id} className="h-20 border-b border-zinc-800/50 relative group">
                     ...
                     {trackClips.map(clip => {
                       ...
                       const isAudio = clip.type === MediaType.AUDIO || (clip.type === MediaType.VIDEO && track.type === 'audio'); // Simple check, though video usually on video track
